...
half _PointSize;
float4x4 _Transform;
float4x4 _DepthCamMatrix;
float4x4 _DepthCamProjection;
...
Varyings Vertex(Attributes input)
{
    ...
    float4 view_pos = mul(_DepthCamMatrix, pos);
    float4 cam_pos = mul(_DepthCamProjection,view_pos);

    float2 screen_pos = cam_pos.xy/cam_pos.w;
    ...
    if((screen_pos.x > -1 && screen_pos.x < 1) && (screen_pos.y > -1 && screen_pos.y < 1.0) && view_pos.z < 0)
    {
        o.position = o.position + float4(1000,1000,0,0);
    }
